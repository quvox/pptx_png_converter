...
/app/
  ...
  │   ├── convert.sh      # 変換スクリプト
  │   ├── extract_notes.py # ノート欄抽出・ファイル名生成スクリプト
...
### ノート欄からのテキスト抽出
- PPTXファイル内のnotesSlide1.xmlから1ページ目のノート欄テキストを抽出
- XMLパースによるテキスト要素の取得
- ファイル名として使用できない文字の自動変換：
  - 特殊文字（`<>:"/\\|?*`）をアンダースコア（`_`）に変換
  - 空白文字をアンダースコア（`_`）に変換
  - 長すぎる場合は50文字に制限
...
### ファイル名決定ロジック
1. **ノート欄にテキストが存在する場合**：
   - ノート欄のテキストをファイル名として使用
   - 同名ファイルが存在する場合は連番を付加（`filename_001.png`）

2. **ノート欄が空または抽出できない場合**：
   - 元のPPTXファイル名をベースとして使用
   - 同名ファイルが存在する場合は連番を付加（`filename_001.png`）

### 重複ファイル名の処理
- 出力先ディレクトリで既存ファイル名をチェック
- 重複する場合は3桁の連番を自動付加（`_001`, `_002`, ...）
